...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromCart(state, action) {
      ...
      state.items = state.items.filter((i) => !sameLine(i, { productId, size, color }))
      persist(state.items)
    },
    clearCart(state) {
      state.items = []
...
export const selectCartSubtotal = (state) =>
  state.cart.items.reduce((sum, item) => sum + Number(item.sellingPrice) * item.quantity, 0)

export const selectCartOriginalTotal = (state) =>
  state.cart.items.reduce(
    (sum, item) => sum + Number(item.originalPrice ?? item.sellingPrice) * item.quantity,
    0
  )
