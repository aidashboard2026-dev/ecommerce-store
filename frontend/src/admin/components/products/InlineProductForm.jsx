...
function LocalVariantForm({ onAdd, existingVariants = [], limits }) {
  ...
  const handleAdd = () => {
    ...
    if (dupExists) {
      toast.error(
        `Variant with size "${form.size}" and color "${form.color || "none"}" already exists`,
      );
      return;
    }
    const newColorNormalized = form.color
...
export default function InlineProductForm({
  ...
  const revokeLocalImages = (imgs) => {
    if (!imgs) return;
    ["thumbnail", "image_front", "image_back", "image_size_chart"].forEach(
      (k) => {
  ...
  const batchSave = async (overrideStatus) => {
    ...
    if (localImages.image_front) {
      imagesToUpload.push({
        file: localImages.image_front.file,
        type: "front",
        setAsPrimary: !localImages.thumbnail,
      });
    }
    if (localImages.image_back) {
      imagesToUpload.push({
        file: localImages.image_back.file,
        type: "back",
        setAsPrimary: false,
      });
    }
    if (localImages.image_size_chart) {
      imagesToUpload.push({
        file: localImages.image_size_chart.file,
        type: "size_chart",
        setAsPrimary: false,
      });
    }
  ...
  const thumbnailUrl = isEdit
    ...
    : localImages.thumbnail?.previewUrl ||
      localImages.image_front?.previewUrl ||
      null;
  const hasThumbnail = isEdit
    ? !!product?.thumbnail
    : !!(localImages.thumbnail || localImages.image_front);
